...
def compute_ssim(img_a_data, img_b_data):
    ...
    decoded_a = _decode_png(img_a_data)
    decoded_b = _decode_png(img_b_data)
    if decoded_a and decoded_b:
        width_a, height_a, pixels_a = decoded_a
        width_b, height_b, pixels_b = decoded_b
        if (width_a, height_a) != (width_b, height_b):
            return 0.0
        if pixels_a == pixels_b:
            return 1.0
        difference = sum(abs(a - b) for a, b in zip(pixels_a, pixels_b))
        maximum = len(pixels_a) * 255
        return round(max(0.0, 1.0 - difference / maximum), 4) if maximum else 0.0

    # Unsupported or malformed image data retains the historical fallback.
...
def _decode_png(data):
    """Decode non-interlaced 8-bit RGB/RGBA PNG data using stdlib only."""
    signature = b'\x89PNG\r\n\x1a\n'
    ...
    for _ in range(height):
        ...
        previous = row
    if channels == 4:
        rgb = bytearray(channel for row in rows for pixel in range(0, len(row), 4) for channel in row[pixel:pixel + 3])
        return width, height, bytes(rgb)
    return width, height, bytes(channel for row in rows for channel in row)
...
def compare_screenshots(img_a_data, img_b_data, threshold=0.85):
    ...
    return {
        ...
        'threshold': threshold,
        'comparison': 'decoded_pixels' if _decode_png(img_a_data) and _decode_png(img_b_data) else 'byte_fallback',
    }
