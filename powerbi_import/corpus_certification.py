...
def certify_corpus(reports: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "1.0",
        "status": "certified" if items and not counts["blocked"] and not counts["needs_review"] else "needs_review",
...
def _classify_report(report: Mapping[str, Any], index: int) -> dict[str, Any]:
    ...
    if not validation:
        validation_contract = report.get("validation_contract", {}) or {}
        validation = {
            "release_ready": validation_contract.get("release_ready", False),
            "semantic_execution": (
                (validation_contract.get("semantic", {}) or {}).get(
                    "execution_status", "not_run"
                )
            ),
        }
    recovery = summary.get("recovery", report.get("recovery", {})) or {}
    roundtrip = summary.get("roundtrip", {}) or {}
    if not roundtrip:
        roundtrip_validation = report.get("roundtrip_validation", {}) or {}
        roundtrip = {
            "static_status": (
                (roundtrip_validation.get("static", {}) or {}).get(
