...
def _extract_pbi_visual_info(visual_json: Dict) -> Dict:
    ...
    if not title:
        ...
        if title_objects and isinstance(title_objects[0], dict):
            props = title_objects[0].get('properties', {})
            literal = ((props.get('text') or {}).get('expr') or {}).get('Literal', {})
            title = literal.get('Value', '')
            if isinstance(title, str) and len(title) >= 2 and title[0] == "'" and title[-1] == "'":
            ...
            for state in query_state.values():
                ...
                for projection in state.get('projections', []) or []:
                    ...
                    for ref_type in ('Column', 'Measure', 'Aggregation'):
                        ref = field.get(ref_type)
                        if not isinstance(ref, dict):
                            continue
                        if ref_type == 'Aggregation':
                            ref = (ref.get('Expression') or {}).get('Column', {})
                        expr = ref.get('Expression', {})
                        src = expr.get('SourceRef', {}) if isinstance(expr, dict) else {}
                        entity = src.get('Entity', '') if isinstance(src, dict) else ''
...
def _diff_worksheet(ws: Dict, pbi_visuals: List[Dict]) -> Dict:
    ...
    try:
        ...
        expected_pbi = resolve_visual_type(mark_type)
    except ImportError:
