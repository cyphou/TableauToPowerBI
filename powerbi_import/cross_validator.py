...
def validate_visual_data_roles(visual_json: Dict[str, Any]) -> Dict[str, Any]:
    """Validate PBIR queryState roles without inferring runtime behavior."""
    from powerbi_import.visual_generator import VISUAL_DATA_ROLES

    visual = visual_json.get("visual", {}) if isinstance(visual_json, dict) else {}
    visual_type = visual.get("visualType", "") if isinstance(visual, dict) else ""
    ...
    if not isinstance(query_state, dict):
        query_state = {}
    allowed_dimensions, allowed_measures = VISUAL_DATA_ROLES.get(visual_type, ([], []))
    allowed = set(allowed_dimensions) | set(allowed_measures)
    findings = []
    invalid_roles = []
    malformed = []
    for role, state in query_state.items():
        ...
        projections = state.get("projections") if isinstance(state, dict) else None
        ...
        for projection in projections:
            ...
            field_kind = next((kind for kind in ("Column", "Measure", "Aggregation") if kind in field), None)
    ...
    return {
        ...
        "malformed_roles": sorted(set(malformed)),
...
def scan_visual_role_contract(project_dir: str) -> Dict[str, Any]:
    """Scan generated PBIR visuals for queryState role evidence."""
    patterns = (
    ...
    for path in sorted({path for pattern in patterns for path in glob.glob(pattern)}):
        ...
        finding["path"] = os.path.relpath(path, project_dir).replace(os.sep, "/")
        finding["visual_id"] = payload.get("name")
        rows.append(finding)
