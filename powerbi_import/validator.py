...
def _split_call_arguments(text, open_pos):
    """Return the closing parenthesis and top-level call arguments."""
    depth = 0
    start = open_pos + 1
    args = []
    quote = None
...
class ArtifactValidator:
    ...
    def auto_fix_dax_leaks(cls, formula):
        ...
        repairs = []
        result = formula
        result, zn_repairs = cls._replace_null_functions(result)
        repairs.extend(zn_repairs)
    ...
    @staticmethod
    def _replace_null_functions(formula):
        """Rewrite ZN/IFNULL calls with balanced, argument-aware DAX."""
        repairs = []
        pattern = re.compile(r'\b(ZN|IFNULL)\s*\(', re.IGNORECASE)
        cursor = 0
        while True:
            match = pattern.search(formula, cursor)
            if not match:
                break
            end, args = _split_call_arguments(formula, match.end() - 1)
            if end is None or not args:
                cursor = match.end()
                continue
            name = match.group(1).upper()
            if name == 'ZN' and len(args) == 1:
                replacement = f'IF(ISBLANK({args[0]}), 0, {args[0]})'
            elif name == 'IFNULL' and len(args) == 2:
                replacement = f'IF(ISBLANK({args[0]}), {args[1]}, {args[0]})'
            else:
                cursor = end + 1
                continue
            formula = formula[:match.start()] + replacement + formula[end + 1:]
            repairs.append(f'{name}(...) → balanced IF(ISBLANK(...))')
            cursor = match.start() + len(replacement)
        return formula, repairs
