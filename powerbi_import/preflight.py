...
def _check_xml_coherence(root: Optional[ET.Element], result: PreflightResult) -> None:
    ...
    referenced_worksheets = set()
    for node in root.iter("zone-contains"):
        match = re.match(r"Worksheet:\s*(.+)$", (node.text or "").strip())
        if match:
            referenced_worksheets.add(match.group(1))
    for node in root.iter():
        target = node.attrib.get("target-sheet", "")
        if target:
            referenced_worksheets.add(target)
    ...
    for tag, code, label in (("worksheet", "duplicate_worksheet_name", "worksheet"),
        ...
        for name in duplicates:
            severity = BLOCKER if label != "worksheet" or name in referenced_worksheets else WARNING
            result.add(severity, code, f"Duplicate {label} name: {name!r}",
