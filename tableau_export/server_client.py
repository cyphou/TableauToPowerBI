...
class TableauServerClient:
    ...
    def _request(self, method, url, headers=None, data=None, json_body=None,
        ...
        try:
            ...
            kwargs = {'headers': hdrs, 'timeout': DEFAULT_REQUEST_TIMEOUT}
        ...
        if os.environ.get('TABLEAU_SSL_NO_VERIFY'):
            logger.warning('TABLEAU_SSL_NO_VERIFY is enabled; TLS verification is disabled')
        ...
        try:
            with urllib.request.urlopen(req, context=ssl_context,
                                        timeout=DEFAULT_REQUEST_TIMEOUT[1]) as resp:
                if stream_to:
                    with open(stream_to, 'wb') as f:
                        while True:
