...
class TestVisualDataRoles(unittest.TestCase):
    def test_valid_bar_roles(self):
        ...
        result = validate_visual_data_roles(visual)
        self.assertEqual(result['status'], 'valid')
        self.assertEqual(result['issues'], [])

    def test_invalid_and_malformed_roles_are_reported(self):
        visual = {
            'visual': {
                'visualType': 'clusteredBarChart',
                'query': {'queryState': {
                    'Values': {'projections': [{'field': {'Column': {}}}]},
                    'Category': {'projections': 'not-a-list'},
                }},
            }
        }
        result = validate_visual_data_roles(visual)
        self.assertEqual(result['status'], 'needs_review')
        self.assertEqual(result['invalid_roles'], ['Values'])
        self.assertIn('Category', result['malformed_roles'])
        self.assertIn('missing required role: Y', result['issues'])
