...
class TestPbirVisualRecovery(unittest.TestCase):
    ...
    def test_scans_report_definition_from_project_root(self):
        with tempfile.TemporaryDirectory() as root:
            visual_root = os.path.join(
                root, "Demo.Report", "definition", "pages", "page1", "visuals", "v1"
            )
            os.makedirs(visual_root)
            with open(os.path.join(visual_root, "visual.json"), "w", encoding="utf-8") as handle:
                json.dump({"visual": {"visualType": "table", "query": {"Column": "Sales"}}}, handle)

            evidence = scan_pbir_visual_recovery(root)

        self.assertEqual(evidence["status"], "scanned")
        self.assertEqual(evidence["visual_count"], 1)
        self.assertEqual(evidence["visuals"][0]["path"], "Demo.Report/definition/pages/page1/visuals/v1/visual.json")
