...
class TestRunningFunctions(unittest.TestCase):
    ...
    def test_comments_do_not_trigger_conversion_validation(self):
        result = convert_tableau_formula_to_dax(
            "// Opportunity Owner's [Sales Value]\nRUNNING_SUM(SUM([Sales]))",
            table_name="Orders", validate_output=True, fallback_on_invalid=True)
        self.assertNotIn("TODO: DAX conversion validation failed", result)
        self.assertNotIn("RUNNING_SUM", result)
        self.assertEqual(validate_dax_expression(result), [])

    def test_comment_stripping_preserves_urls(self):
        result = convert_tableau_formula_to_dax(
            "// documentation\n\"https://example.test/item/\" + [Opportunity Id]",
            table_name="Owned By", validate_output=True, fallback_on_invalid=True)
        self.assertIn("https://example.test/item/", result)
        self.assertNotIn("TODO: DAX conversion validation failed", result)
