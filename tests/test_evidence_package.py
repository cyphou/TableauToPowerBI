...
class TestEvidencePackage(unittest.TestCase):
    ...
    def test_zip_includes_related_quality_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(path) as archive:
                ...
                self.assertIn("Release readiness:", readme)
                self.assertIn("Release claim: static_corpus_only", readme)
    ...
    def test_payload_contains_operator_summary_counts(self):
        ...
        self.assertEqual(payload["certification"]["release_claim"], "static_corpus_only")
        self.assertIn("release_readiness", payload)
        self.assertEqual(payload["release_readiness"]["release_claim"], "static_corpus_only")
        self.assertEqual(payload["release_readiness"]["gates"]["runtime_desktop"]["status"], "not_run")
