...
class TestScreenshotComparison(unittest.TestCase):
    ...
    def test_decodes_pixels_instead_of_compressed_bytes(self):
        from powerbi_import.equivalence_tester import compare_screenshots
        red = _png_pixel(255, 0, 0)
        almost_red = _png_pixel(254, 0, 0)
        result = compare_screenshots(red, almost_red)
        self.assertEqual(result['comparison'], 'decoded_pixels')
        self.assertGreater(result['ssim'], 0.99)

    def test_different_decoded_pixels_fail_strict_threshold(self):
        from powerbi_import.equivalence_tester import compare_screenshots
        result = compare_screenshots(_png_pixel(0, 0, 0), _png_pixel(255, 255, 255))
        self.assertEqual(result['comparison'], 'decoded_pixels')
        self.assertFalse(result['passed'])
