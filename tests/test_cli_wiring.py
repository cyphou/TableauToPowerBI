...
class TestWarningSummaryParsing(unittest.TestCase):
    """Regression tests for partial-batch warning reporting."""

    def test_extract_failed_items_from_batch_log(self):
        from web.light_ui import LightMigrationUI
        log_text = """
        ...
        [FAIL] Security_Test
        """
        items = LightMigrationUI._extract_failed_items(log_text)
        self.assertEqual(items, ['HR_Analytics', 'Security_Test'])

    def test_build_warning_summary_mentions_failed_items(self):
        from web.light_ui import LightMigrationUI
        ui = object.__new__(LightMigrationUI)
        ui._all_logs = [
            "BATCH MIGRATION SUMMARY\n",
            "Succeeded: 9\n",
            "Failed: 2\n",
        ...
        summary = ui._build_warning_summary()
        self.assertIn('Succeeded: 9', summary)
        self.assertIn('Failed: 2', summary)
        self.assertIn('HR_Analytics', summary)
        self.assertIn('Security_Test', summary)


class TestLightUIPresetCommands(unittest.TestCase):
    """Ensure visible UI presets emit their documented CLI flags."""

    def test_quality_preset_enables_quality_report(self):
        ...
        ui = object.__new__(LightMigrationUI)
        ui.repo_root = Path.cwd()
        ui.migrate_script = ui.repo_root / 'migrate.py'
        ui.preset_var = Value('Quality')
        ui.source_var = Value('examples/tableau_samples')
        ui.output_var = Value('artifacts/test_quality_preset')
        ui.output_format_var = Value('Power BI')
        ui.quality_policy_var = Value('enterprise')
        ui.verbose_var = Value(False)
        ui.global_assess_var = Value(False)
        ui.prep_lineage_only_var = Value(False)

        command = ui._build_command()

        self.assertIn('--quality-report', command)
        self.assertIn('--quality-policy', command)
        self.assertIn('enterprise', command)

    def test_qa_preset_enables_qa_suite(self):
        from pathlib import Path
        from web.light_ui import LightMigrationUI
        ...
        ui.repo_root = Path.cwd()
        ui.migrate_script = ui.repo_root / 'migrate.py'
        ui.preset_var = Value('QA')
        ui.source_var = Value('examples/tableau_samples')
        ui.output_var = Value('artifacts/test_qa_preset')
        ui.output_format_var = Value('Power BI')
        ui.quality_policy_var = Value('report')
        ui.verbose_var = Value(False)
        ui.global_assess_var = Value(False)
        ui.prep_lineage_only_var = Value(False)
        ...
        self.assertNotIn('--quality-report', command)

    def test_parity_preset_enables_parity_scan(self):
        from pathlib import Path
        from web.light_ui import LightMigrationUI
        ...
        ui.repo_root = Path.cwd()
        ui.migrate_script = ui.repo_root / 'migrate.py'
        ui.preset_var = Value('Parity')
        ui.source_var = Value('examples/tableau_samples')
        ui.output_var = Value('artifacts/test_parity_preset')
        ui.output_format_var = Value('Power BI')
        ui.quality_policy_var = Value('report')
        ui.verbose_var = Value(False)
        ui.global_assess_var = Value(False)
        ...
        self.assertIn('--parity', command)
        self.assertNotIn('--qa', command)

    def test_portfolio_preset_enables_bulk_assessment(self):
        from pathlib import Path
        from web.light_ui import LightMigrationUI
        ...
        ui.repo_root = Path.cwd()
        ui.migrate_script = ui.repo_root / 'migrate.py'
        ui.preset_var = Value('Portfolio')
        ui.source_var = Value('examples/tableau_samples')
        ui.output_var = Value('artifacts/test_portfolio_preset')
        ui.output_format_var = Value('Power BI')
        ui.quality_policy_var = Value('report')
        ui.verbose_var = Value(False)
        ui.global_assess_var = Value(False)
        ...
        self.assertIn('--bulk-assess', command)
        self.assertIn('examples/tableau_samples', command)
