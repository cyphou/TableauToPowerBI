...
def _print_batch_summary(batch_results, batch_duration, migrated_root):
    ...
    if quality_results:
        ...
        try:
            from powerbi_import.migration_quality import build_consolidated_quality_html
            quality_json_paths = [
                result.get('quality_json')
                for result in wb_results.values()
                if result.get('quality_json')
            ...
            consolidated_path = os.path.join(
                migrated_root, 'MIGRATION_QUALITY_CONSOLIDATED.html')
            build_consolidated_quality_html(quality_json_paths, consolidated_path)
            print(f"  Consolidated quality report: {consolidated_path}")
        except (ImportError, OSError, ValueError) as exc:
            logger.warning("Consolidated quality report failed: %s", exc)
