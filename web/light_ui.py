...
class LightMigrationUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        if getattr(sys, "frozen", False):
            self.repo_root = Path(getattr(sys, "_MEIPASS", Path(sys.executable).parent))
        else:
    ...
    def _load_settings(self) -> None:
        ...
        if preset in {"Assess", "Portfolio", "Migrate", "Fabric", "Quality", "QA", "Parity", "M Coverage", "Server", "Lineage"}:
    ...
    def _build_ui(self) -> None:
        ...
        self.workflow_hint = tk.Label(
            setup_card,
            text="Choose Assess, Portfolio, Migrate, Fabric, Quality, QA, Parity, M Coverage, Server, or Lineage.",
            fg=self.ui["muted"],
            bg=self.ui["surface"],
            anchor="w",
            justify="left",
    ...
    def _show_more_tasks(self) -> None:
        menu = tk.Menu(self.root, tearoff=False, bg="#ffffff", fg=self.ui["text"],
                       activebackground=self.ui["chip_hover"], activeforeground=self.ui["primary"],
                       relief="solid", bd=1)
        for task, icon in (("Portfolio", "◫"), ("QA", "✓"), ("Parity", "◌"), ("M Coverage", "▦"), ("Server", "☁"), ("Lineage", "⌁")):
    ...
    def _apply_preset(self) -> None:
        ...
        try:
            if preset == "Migrate":
                ...
                self.prep_lineage_only_var.set(False)
                self.verbose_var.set(True)
                self.workflow_hint.configure(text="Assess portfolio readiness, merge opportunities, Prep lineage, effort, and migration waves without generating projects.")
            elif preset == "Quality":
                self.mode_var.set("batch")
                self.assess_only_var.set(False)
                self.global_assess_var.set(False)
                ...
                self.prep_lineage_only_var.set(False)
                self.verbose_var.set(True)
                self.workflow_hint.configure(text="Run the unified QA suite with artifact repair, comparison, and governance evidence.")
            elif preset == "Parity":
                self.mode_var.set("batch")
                self.assess_only_var.set(False)
                self.global_assess_var.set(False)
                self.prep_lineage_only_var.set(False)
                self.verbose_var.set(True)
                self.workflow_hint.configure(text="Scan Tableau feature usage and generate a parity scorecard before migration.")
    ...
    def _build_command(self) -> list[str]:
        ...
        if getattr(sys, "frozen", False):
            cmd = [sys.executable, "--run-engine", str(self.migrate_script)]
        else:
        ...
        elif self.preset_var.get() == "Portfolio":
            cmd += ["--bulk-assess", source]
        ...
        if self.preset_var.get() == "Quality":
            cmd.append("--quality-report")
        elif self.preset_var.get() == "QA":
            cmd.append("--qa")
        elif self.preset_var.get() == "Parity":
            cmd.append("--parity")
    ...
    @staticmethod
    def _extract_failed_items(log_text: str) -> list[str]:
        ...
        for line in log_text.splitlines():
            ...
            if not stripped:
                continue
            if re.match(r"^(Succeeded|Failed):\s*\d+", stripped, re.IGNORECASE):
                continue
        ...
        if not failed_items:
            for line in log_text.splitlines():
                ...
                if re.match(r"^(Succeeded|Failed):\s*\d+", stripped, re.IGNORECASE):
                    continue
                match = re.search(r"(?:^|\s)([A-Za-z0-9_\-]+)\s+\(failed\)|(?:^|\s)([A-Za-z0-9_\-]+)\s+failed\b", stripped, re.IGNORECASE)
                if match:
    ...
    def _build_warning_summary(self) -> str:
        log_text = "\n".join(self._all_logs)
        success_match = re.search(r"Succeeded:\s*(\d+)", log_text, re.IGNORECASE)
        failed_match = re.search(r"Failed:\s*(\d+)", log_text, re.IGNORECASE)
        failed_items = self._extract_failed_items(log_text)

        summary_lines = []
        if success_match or failed_match:
            summary_lines.append(f"Succeeded: {success_match.group(1) if success_match else '0'}")
            summary_lines.append(f"Failed: {failed_match.group(1) if failed_match else '0'}")
        if failed_items:
            summary_lines.append(f"Failed items: {', '.join(failed_items)}")
        else:
            summary_lines.append("Review the run log for the affected workbooks.")
        return "\n".join(summary_lines)
