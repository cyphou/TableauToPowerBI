...
def slide_executive_summary(prs):
    ...
    roi_cards = [
        ...
        ("🎯", "5/6", "Openability", "one real-world blocker set"),
...
def slide_migration_results(prs):
    ...
    cards = [
        ("6/6", "PBIP Projects\nGenerated", SUCCESS),
        ("5/6", "Openability\nPassed", PBI_BLUE),
        ("1", "Blocked\n(real-world)", RGBColor(0xA4, 0x26, 0x2C)),
        ("150", "Focused\nTests", PURPLE),
        ("v48", "Roadmap\nTrack", TEAL),
    ]
    card_w = Inches(2.2)
    ...
    _add_text(slide, Inches(6.5), Inches(3.2), Inches(6.5), Inches(0.5),
              "Blocked but generated: real-world workbook", 18, RGBColor(0xA4, 0x26, 0x2C), bold=True)
    blockers = [
        "Missing semantic reference (example placeholder)",
        "Missing visual measure (example placeholder)",
...
def slide_next_steps(prs):
    ...
    steps = [
        ...
        "2.  Review and remediate the two real-world reference blockers",
