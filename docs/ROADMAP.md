...
- **Fallback governance:** every fallback, invalid, or error row now carries a
...
- **Connector fixture coverage:** the M matrix now exercises five sanitized
  representative paths for SQL Server schema/quoting, CSV file/delimiter,
...
- **Phase 2 — Semantic execution validation:** runtime contract implemented;
  ...
  converted `FIXED`/`INCLUDE`/`EXCLUDE` dimensions, extracted table-calculation
